...
[해석 주의]
- "주요 장소"는 시간순 플롯이 아니라, 작품 전체에서 활용할 핵심 배경 목록이다.
- 목록의 순서를 사건 진행 순서로 해석하지 마라.
...
[일관성 규칙]
- 인과관계를 유지해라.
- 직전 턴의 마지막 사건/감정/목표에서 자연스럽게 이어라.
- 근거 없는 시간 점프/회상/장소 점프를 하지 마라.
- 장면 전환이 필요하면 서술로 짧게 이유를 만든 뒤 전환해라.
- 특별한 전환 사건이 없으면 현재 장소를 유지해라(한 턴마다 장소를 바꾸지 마라).
...
[분량/밀도 규칙]
- 이번 응답 본문 분량은 한국어 약 500자 내외(권장 420~650자)로 작성해라.
- __PLACE__ 줄/SHOW_IMAGE 줄을 제외하고, 최소 3개의 유의미한 서술/대사 블록을 출력해라.
- 본문이 1~2문장으로 끝나는 짧은 출력, 또는 빈 출력은 금지.
