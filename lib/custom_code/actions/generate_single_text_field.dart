...
- 프롤로그는 단 1개 버전.
- 900~1500자. 도입→확대→절벽.
- 출력은 반드시 아래 "유저 표시용 포맷"만 사용.
- 각 줄은 반드시 줄바꿈으로 분리(한 줄에 한 항목).
- 따옴표(" ' ) 절대 사용 금지.
- 아래에 없는 라벨/태그/설명/주석 절대 금지.

[유저 표시용 포맷]
1) 첫 줄(필수, 1번만):
배경이미지: 장소명

2) 이후 줄(여러 번 가능, 아래 3종만 허용)
- 상황이미지: 상황명
- 이름(감정): 대사
- (라벨 없는 문장): 내레이션 문장

[규칙]
- 장소명은 (있다면) [사용 가능한 장소 목록]에서만 1개 선택.
- 상황명은 <CTX> 안의 "[보유 상황 태그]" 목록에서만 선택(완전 동일 문자열).
- 감정은 <CTX> 안의 "[보유 감정 태그]"에서 선택(없거나 애매하면 무감정 사용).
- 대사는 반드시 캐릭터 이름만 사용(캐릭터 목록 밖 이름 금지).
- 내레이션 줄에는 콜론(:) 사용 금지(파싱 혼동 방지).
""";
  } else {
    selectedModel = 'solar-mini';
    specificInstruction = """
