...
""";
    ...
    // [프롤로그]: 파싱(Parsing)을 위한 특수 포맷 적용
    specificInstruction = """
- 사건이 시작되는 도입부를 흥미진진하게 서술하세요.
- **반드시 아래의 3가지 형식을 섞어서 작성해야 합니다.** (파싱을 위해 엄격히 준수하세요)
1. **지문(Narration):** 일반적인 서술문은 그대로 씁니다.
2. **대사(Dialogue):** 캐릭터가 말을 할 때는 반드시 **"이름 | 대사내용"** 형식을 쓰세요. (가운데 | 파이프 기호 필수)
   - 예시: 리온 | "이곳이 바로 마왕성인가?"
3. **이미지(Image):** 특정 상황이나 배경 묘사가 필요할 때 **"[Image: 조건]"** 형식을 쓰세요.
   - 예시: [Image: 불타는 성]
   - 예시: [Image: 비 내리는 숲]
...
차가운 빗줄기가 내리고 있었다.
탐정 | "이런 날씨에 살인이라니."
그는 바닥에 떨어진 증거물을 주워들었다.
[Image: 피 묻은 손수건]

- 위 형식을 지켜서 한국어로 자연스럽게 작성하세요.
""";
