...
[주요 장소 해석 규칙]
- "주요 장소"는 스토리 시간순 목록이 아니라, 작품에서 사용할 수 있는 핵심 배경의 나열이다.
- 따라서 목록의 위에서 아래 순서를 사건의 시간 순서로 해석하지 마라.
- 프롤로그는 장면 하나에 집중해서 장소 1곳만 선택해 시작하라.
...
${hasConfiguredChars ? """
- 설정된 캐릭터가 있으므로, 반드시 설정된 캐릭터 중 최소 1명이 'A형식 대사'를 말하게 해라.
...
[분량]
- 한국어 450~650자(목표 500자 내외)
- 장소는 프롤로그 내내 유지(이동/전환 묘사 금지)

[이야기의 생성]
- 이제 위 규칙들을 지키면서 스토리의 첫 시작을 흥미롭게 생성하라.
- 어색하지 않게 자연스럽게 이야기를 시작하라.
- 설정된 캐릭터를 전원 등장시킬 필요는 없다. 장면에 필요한 인물만 사용하라.
- 캐릭터를 순서대로 한 줄씩 말시키는 라운드로빈 형태를 금지한다.
- 한 장면에서 실제로 말이 필요한 인물만 대사하게 하고, 대사 인물 수는 최대 2명으로 제한하라.
- 대사만 연속하지 말고, 짧은 내레이션으로 상황/긴장/행동을 연결하라.
- 내레이션 비중을 대사보다 높게 유지하라(최소 60%).
- 첫 장면에서 독자가 다음 장면을 궁금해할 갈등의 씨앗 1개를 분명히 남겨라.
"""
      .trim();
